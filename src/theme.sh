...
### Load Options
border_style_active_pane=$(get_tmux_option "@theme_active_pane_border_style" "fg=${PALETTE['dark5']}")
border_style_inactive_pane=$(get_tmux_option "@theme_inactive_pane_border_style" "fg=${PALETTE[bg_highlight]}")
...
IFS=',' read -r -a plugins <<<"$(get_tmux_option "@theme-plugins" "")"
...
for plugin in "${plugins[@]}"; do
	...
	if [ ! -f "${CURRENT_DIR}/plugin/${plugin}.sh" ]; then
		...
		separator_start="#[fg=${PALETTE[$accent_color]},bg=${PALETTE[bg_highlight]}]${right_separator}#[none]"
		separator_end="#[fg=${PALETTE[bg_highlight]},bg=${PALETTE[$accent_color]}]${right_separator}#[none]"
		separator_icon_start="#[fg=${PALETTE[$accent_color_icon]},bg=${PALETTE[bg_highlight]}]${right_separator}#[none]"
		separator_icon_end="#[fg=${PALETTE[$accent_color]},bg=${PALETTE[$accent_color_icon]}]${right_separator}#[none]"

		plugin_output="#[fg=${PALETTE[white]},bg=${PALETTE[$accent_color]}]$(load_plugin)#[none]"
		plugin_output_string=""

		plugin_icon_output="${separator_icon_start}#[fg=${PALETTE[white]},bg=${PALETTE[$accent_color_icon]}]${plugin_icon}${separator_icon_end}"
